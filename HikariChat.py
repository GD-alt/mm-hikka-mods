    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "warns_punishment",
                'mute',
                lambda: "Which punishment will be applied after warns limit",
                validator=loader.validators.Choice(['mute', 'ban', 'kick']),
            ),
            loader.ConfigValue(
                "warns_punishment_length",
                '7 days',
                lambda: "How long punishment will be applied after warns limit",
                validator=loader.validators.RegExp(r'(\d+ (day(s)?|hour(s)?|minute(s)?|second(s)?)|forever)'),
            ),
        )
    ...
    async def warncmd(self, message: Message):
        ...
        if len(warns) >= self.config["warns_limit"]:
            ...
            for c in chats:
                ...
                timer = self.config['warns_punishment_length']
                if timer == 'forever':
                    unit = 'forever'
                    length = 0
                else:
                    length, unit = int(timer.split(' ')[0]), timer.split(' ')[1]
                if 'hour' in unit:
                    addition = length * 60 * 60
                elif 'day' in unit:
                    if length > 366:
                        addition = 366 * 24 * 60 * 60
                    else:
                        addition = length * 60 * 60 * 24
                elif 'minute' in unit:
                    addition = length * 60
                elif 'second' in unit:
                    if length < 30:
                ...
                if self.config["warns_punishment"] == 'ban':
                    await self._client(
                        EditBannedRequest(
                            c,
                            user,
                            ChatBannedRights(
                                until_date=time.time() + addition,
                                view_messages=True,
                            ),
                        )
                    )
                    performed = 'banned'
                elif self.config["warns_punishment"] == 'mute':
                    await self._client(
                        EditBannedRequest(
                            c,
                            user,
                    ...
                    performed = 'muted'
                elif self.config["warns_punishment"] == 'kick':
                    await self._client(
                        EditBannedRequest(
                            c,
                            user,
                    ...
                    performed = 'kicked'
                result = f'{performed} him for {length} {unit}' if unit != 'forever' and self.config["warns_punishment"] != 'kick' else f'{performed} him out' if self.config["warns_punishment"] == 'kick' else f'{performed} him forever'
